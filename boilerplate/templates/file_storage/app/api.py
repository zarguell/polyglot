...
async def upload_file(
    ...
    db: DbDeps = Depends(),  # noqa: B008
    current_user: CurrentUser = Depends(),  # noqa: B008
    service: StorageService = Depends(_get_storage_service),  # noqa: B008
) -> FileUploadResponse:
...
async def download_file(
    file_id: uuid.UUID,
    db: DbDeps = Depends(),  # noqa: B008
    current_user: CurrentUser = Depends(),  # noqa: B008, ARG001 (implicit auth gate)
    service: StorageService = Depends(_get_storage_service),  # noqa: B008
) -> StreamingResponse:
